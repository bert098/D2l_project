# bin_and_hex.asm
# ENCM 369 Winter 2018 Lab 4 Exercise E Partial Solution
#
...
write_in_binary:
	...
	ori	$t2, $zero, '0'		# digit0 = '0'
	ori	$t3, $zero, '1'		# digit1 = '1'
	ori	$t4, $zero, '_'		# under = '_'
	lui	$t1, 0x8000		# mask = 0x8000
	ori	$t1, $t1, 0x0000	# mask = 0x80000000
	addi 	$t0, $zero, 31		# bn = 31
L1:
	slt	$t5, $t0, $zero		# $t5 = (bn < 0)
	bne	$t5, $zero, L2 		# if ($t5 == 0) goto L2 
	and	$t6, $a1, $t1		# $t6 = word & mask
	beq 	$t6, $zero, L3		# if ($t6 == 0) goto L3 
	sb	$t3, 0($a0)		# *str = digit1
	j	L4
L3:
	sb 	$t2, 0($a0)		# *str = digit0
L4:
	addi 	$a0, $a0, 1		# str++
	slt	$t7, $zero, $t0		# $t7 = ( 0 < bn)
	beq	$t7, $zero, L5		# if ($t7 == 0) goto L5
	andi	$t8, $t0, 3		# $t8 = bn & 3 
	bne	$t8, $zero, L5		# if ($t8 != 0) goto L5
	sb	$t4, 0($a0) 		# *str = under 
	addi	$a0, $a0, 1 		# str++
L5:
